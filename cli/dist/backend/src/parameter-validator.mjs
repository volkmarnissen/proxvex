export class ParameterValidator {
    validate(input) {
        const errors = [];
        const warnings = [];
        const { params, parameterDefs, selectedAddons, availableAddons, stackId, availableStacks, } = input;
        ...
        for (const p of params) {
            ...
            if (def.type === "number") {
                ...
                if (def.enumValues && def.enumValues.length > 0) {
                    const validValues = def.enumValues.map((ev) => typeof ev === "string" ? ev : String(ev.value));
            ...
            for (const addonId of selectedAddons) {
                ...
                if (input.applicationParamIds) {
                    ...
                    if (addon?.required_parameters?.length) {
                        const missing = addon.required_parameters.filter((id) => !input.applicationParamIds.has(id));
                        if (missing.length > 0) {
                            errors.push({
                                field: "addons",
...
//# sourceMappingURL=parameter-validator.mjs.map