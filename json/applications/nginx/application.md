...
A minimal `default.conf` is written automatically on first installation if the volume is empty. Permissions are fixed during installation and reconfigure.

To update the configuration after deployment:

1. **Reconfigure** — Change parameters or addons, the container is cloned with existing volumes. Permissions are fixed automatically.
2. **Direct edit** — Edit files in the volume directory on the PVE host. After adding or changing files:
