#!/bin/bash
# Reconfigure oci-lxc-deployer with addon-ssl (self-signed certificate).
# After this, the deployer runs on HTTPS (port 3443).
#
# Prerequisites:
#   - oci-lxc-deployer is running (HTTP on port 3080)
#   - CA certificate exists (setup-acme.sh generates it)
#
# Usage:
#   ./production/setup-deployer-ssl.sh
...
# --- Step 4: Reconfigure with SSL ---
echo ""
echo "=== Step 4: Reconfigure oci-lxc-deployer with self-signed certificate ==="

cat > /tmp/deployer-ssl-params.json <<EOF
{
...
rm -f /tmp/deployer-ssl-params.json
...
if [ "$https_done" = "true" ]; then
  ...
  echo "  Check container logs for certificate errors."
fi
