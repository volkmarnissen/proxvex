#!/bin/sh
# install-proxvex.sh
# Minimal installation script for proxvex as an LXC container on Proxmox
...
OCI_OWNER="${OCI_OWNER:-proxvex}"
OWNER="${OWNER:-proxvex}"
#OWNER="proxvex"
REPO="proxvex"
BRANCH="${BRANCH:-main}"
OCI_IMAGE="ghcr.io/${OCI_OWNER}/proxvex:latest"
...
execute_script_from_github() {
  ...
  case "$path" in
    ...
    json/shared/scripts/pre_start/conf-create-storage-volumes-for-lxc.sh)
      for _lib_name in pve-common.sh vol-common.sh; do
        if [ -n "$LOCAL_SCRIPT_PATH" ] && [ -f "${LOCAL_SCRIPT_PATH}/json/shared/scripts/library/${_lib_name}" ]; then
          lib_content=$(cat "${LOCAL_SCRIPT_PATH}/json/shared/scripts/library/${_lib_name}")
        else
          lib_url="https://raw.githubusercontent.com/${OWNER}/${REPO}/${BRANCH}/json/shared/scripts/library/${_lib_name}"
          lib_content=$(curl -fsSL "$lib_url")
        fi
        script_content=$(printf '%s\n\n%s' "$lib_content" "$script_content")
      done
...
Installs proxvex as an LXC container from OCI image on a Proxmox host.
...
Options:
  ...
  --hostname <name>     Container hostname. Default: proxvex
...
if ! nc -z localhost 22 2>/dev/null && ! timeout 2 nc -z localhost 22 2>/dev/null; then
  ...
  # Write proxvex drop-in configuration (matches ssh.mts getInstallSshServerCommand)
  cat > /etc/ssh/sshd_config.d/proxvex.conf <<'SSHCONF'
...
SSHCONF
  ...
  if [ ! -f /etc/ssh/sshd_config.d/proxvex.conf ]; then
    log "Adding proxvex SSH configuration..."
...
icon_url="https://raw.githubusercontent.com/${OWNER}/${REPO}/${BRANCH}/json/applications/proxvex/icon.svg"
...
if [ -z "$container_pubkey" ]; then
  ...
  ssh-keygen -t ed25519 -f "${ssh_dir}/id_ed25519" -N "" -C "proxvex@auto-generated" \
    >/dev/null 2>&1
